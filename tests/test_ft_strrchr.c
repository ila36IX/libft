...
TestCase strrchr_tests[] = {
	...
	{5, "Find fisrt character", "Heatblast", 'H'},
...
int run_strrchr_tests(int debug)
{
	int num_tests = sizeof(strrchr_tests) / sizeof(strrchr_tests[0]);
	int OK = TRUE;

	for (int i = 0; i < num_tests; i++) {
		TestCase test_case = strrchr_tests[i];
		char *p = strrchr(test_case.s, test_case.c);
		char *_p = ft_strrchr(test_case.s, test_case.c);
		if (p == _p)
		{
			if (debug)
